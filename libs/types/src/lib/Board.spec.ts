...
describe('Board class', () => {
  ...
  test('setters and getters work correctly', () => {
    ...
    expect(board.elements[0]).toBeInstanceOf(Rectangle);
  });
  ...
  test('from method creates a new instance from existing board', () => {
    ...
    existingBoard.elements = [new Element(), new Rectangle()];
    ...
    expect(newBoard.elements.length).toBe(2);
    expect(newBoard.elements[0]).toBeInstanceOf(Element);
    expect(newBoard.elements[1]).toBeInstanceOf(Rectangle);
  });
  ...
  test('toJSON method converts board to JSON', () => {
    ...
    board.elements = [new Element(), new Rectangle()];
